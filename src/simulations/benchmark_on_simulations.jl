...
"""
    create_star_tree(seqnames::Vector{String})

Create a simple star tree string from sequence names.
"""
function create_star_tree(seqnames::Vector{String})
    if length(seqnames) == 1
        return "$(seqnames[1]):0.1;"
    end
    
    # Create star tree with equal branch lengths
    branch_length = 0.1
    tree_parts = ["$(seqnames[1]):$branch_length"]
    
    for i in 2:length(seqnames)
        push!(tree_parts, "$(seqnames[i]):$branch_length")
    end
    
    return "(" * join(tree_parts, ",") * "):0.1;"
end

"""
    summarize_benchmark_results(results::Dict{String, Dict{String, Any}}; 
                               verbosity::Int=1)

Summarize the results of a FUBAR benchmark run.

# Arguments
- `results::Dict{String, Dict{String, Any}}`: Results from run_fubar_benchmark
- `verbosity::Int=1`: Control level of output messages

# Returns
- `DataFrame`: Summary statistics for each directory and method
"""
function summarize_benchmark_results(results::Dict{String, Dict{String, Any}}; 
                                   verbosity::Int=1)
    
    summary_data = []
    
    for (dirname, dir_results) in results
        for (method_name, method_results) in dir_results
            if haskey(method_results, :error)
                push!(summary_data, (
                    directory=dirname,
                    method=method_name,
                    status="error",
                    error_message=string(method_results[:error]),
                    positive_sites=0,
                    purifying_sites=0,
                    total_sites=0
                ))
                ...
                if haskey(method_results, :results)
                    df_results = method_results[:results]
                    
                    # Count positive and purifying selection sites
                    positive_sites = 0
                    purifying_sites = 0
                    
                    if hasproperty(df_results, :sel)
                        positive_sites = count(x -> x == "Positive", df_results.sel)
                        purifying_sites = count(x -> x == "Purifying", df_results.sel)
                    elseif hasproperty(df_results, :positive_posterior)
                        positive_sites = count(x -> x > 0.95, df_results.positive_posterior)
                        purifying_sites = count(x -> x > 0.95, df_results.purifying_posterior)
                    end
                    ...
                    push!(summary_data, (
                        ...
                        purifying_sites=purifying_sites,
                        total_sites=nrow(df_results)
                    ))
                end
            end
        end
